...
def flash_attn_varlen_func(q, k, v, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k, dropout_p=0.0, causal=True):
    """
    Alternative function for flash_attn_varlen_func on NVIDIA TITAN RTX (Turing).
    Supports GQA/MQA by expanding K and V heads to match Q.
    """
    batch_size = len(cu_seqlens_q) - 1
    original_dtype = q.dtype
    
    # Input shape: [Total_Seq, Head, Dim]
    head_q = q.shape[1]
    head_k = k.shape[1]
    dim = q.shape[2]
    
    # Calculate group size for GQA (e.g., 8 heads / 2 heads = 4)
    num_groups = head_q // head_k

    # 1. Restore Packed tensors and handle GQA/MQA expansion
    q_padded = torch.zeros((batch_size, head_q, max_seqlen_q, dim), device=q.device, dtype=original_dtype)
    k_padded = torch.zeros((batch_size, head_q, max_seqlen_k, dim), device=k.device, dtype=original_dtype)
    v_padded = torch.zeros((batch_size, head_q, max_seqlen_k, dim), device=v.device, dtype=original_dtype)
    ...
    for i in range(batch_size):
        ...
        # Prepare Q
        q_padded[i, :, :end_q-start_q, :] = q[start_q:end_q].transpose(0, 1)
        
        # Prepare K, V with GQA expansion: [Head_K, Seq, Dim] -> [Head_Q, Seq, Dim]
        curr_k = k[start_k:end_k].transpose(0, 1) # [Head_K, Seq, Dim]
        curr_v = v[start_k:end_k].transpose(0, 1)
        
        if num_groups > 1:
            # Repeat heads to match head_q
            curr_k = curr_k.repeat_interleave(num_groups, dim=0)
            curr_v = curr_v.repeat_interleave(num_groups, dim=0)
            
        k_padded[i, :, :end_k-start_k, :] = curr_k
        v_padded[i, :, :end_k-start_k, :] = curr_v
    ...
    # 3. Re-convert to Packed format
    output = output.transpose(1, 2).contiguous() # [Batch, Seq, Head, Dim]
    out_packed = []
    for i in range(batch_size):
        out_packed.append(output[i, :cu_seqlens_q[i+1]-cu_seqlens_q[i]])
    
    return torch.cat(out_packed, dim=0)
