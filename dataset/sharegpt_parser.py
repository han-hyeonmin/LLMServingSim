...
# --------- HF token ----------
# Prompt for HF_TOKEN in terminal (input hidden for security)
    ...
    while request_count < max_requests:
        if not fix_len:
            ...
            if (
                ...
                or len(output_tokens) > max_output_length
                or len(input_tokens) + len(output_tokens) > max_kv_length
            ):
                continue
        else:
            # fixed length inputs/outputs
            vocab_size = (
                tokenizer.vocab_size if hasattr(tokenizer, "vocab_size") else 32000
            )
            input_tokens = [
                random.randint(0, vocab_size - 1) for _ in range(fix_input_length)
            ]  # random token ids
            output_tokens = [
                random.randint(0, vocab_size - 1) for _ in range(fix_output_length)
            ]  # random token ids
